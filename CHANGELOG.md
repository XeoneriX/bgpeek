...
- **`sqlparse>=0.6.0`** — CVE-2026-59893, CVE-2026-54284, CVE-2026-71491 and CVE-2026-84305 (CPU-exhaustion denial of service in the lexer, statement grouping and reindent paths) and CVE-2026-59894 (backslashes were not escaped in the python/php output formats, letting crafted SQL break out of the generated string literal). Reaches bgpeek transitively through `yoyo-migrations`, which only parses the migration files shipped with bgpeek itself — no user-supplied SQL is ever passed to it — so none of these were reachable here; bumped to keep the weekly scan green. `sqlparse.split()` output is byte-identical to 0.5.5 across all shipped migrations. Carried as an explicit direct floor.
- **`pip>=26.2`** (dev extra only) — CVE-2026-13346: pip mishandled doubly-encoded package URLs, letting a malicious package index install files outside the target directory. The locked pip is pulled in by `pip-audit` for the Security workflow and is not installed into bgpeek's runtime virtualenv. The separate system pip in the base image is covered by the runtime-image change below. 26.2 also fixes the three older pip advisories the Security workflow had been ignoring, so the ignore list is now down to paramiko PYSEC-2026-2858 alone.
- **`anyio>=4.14.2`** — CVE-2026-63374 (CRITICAL): `anyio.streams.tls.TLSStream` passed the target hostname to the TLS layer without IDNA 2008 normalisation, so a name was encoded under IDNA 2003 rules, which map distinct Unicode labels onto the same A-label (the classic case being `ß` → `ss`). Certificate validation then ran against the folded name, letting a certificate issued for the lookalike domain be accepted for the intended one. Unlike the other entries in this block this one **is** reachable in bgpeek: httpcore selects its anyio backend under asyncio and its `start_tls` calls `TLSStream.wrap`, so every outbound HTTPS request — RPKI validator, webhook delivery, log shipper, OIDC discovery and token exchange — went through the affected path. Exploitation still requires an operator to have configured one of those endpoints as an internationalised domain whose IDNA 2003 and IDNA 2008 encodings differ, and an attacker holding a valid certificate for the folded name, so the practical exposure on an ASCII hostname deploy is nil. 4.14.2 routes the hostname through `idna2008_resolve`. The same release also fixes CVE-2026-64847 (an `anyio.to_process` worker blocks indefinitely on undrained stderr), which is not reachable here — neither bgpeek nor any locked dependency uses the process pool. Reaches bgpeek transitively through `httpx`, `starlette` and `watchfiles`, and is carried as an explicit direct floor alongside the other transitive pins. The lock resolves to 4.15.1.
- **Runtime image no longer ships pip.** The `python:3.12-slim` base carries a system pip (25.0.1) that bgpeek never uses — the app runs from `/opt/venv`, which has no pip — but it still showed up in image scans with five medium advisories (CVE-2026-13346, CVE-2026-8643, CVE-2026-6357, CVE-2026-3219, CVE-2025-8869) and one low (CVE-2026-1703). None were reachable, since nothing in the container runs pip. The runtime stage now uninstalls it and deletes the bundled `ensurepip` wheel so a known-vulnerable pip cannot be reinstalled from inside the container. `python -m venv --without-pip` still works; `python -m ensurepip` no longer does.
...
- **Dependency security sweep — weekly `pip-audit` + Trivy scan is now clean.** Bumped every dependency carrying a disclosed, fixed CVE: `pyjwt>=2.13.0` (CVE-2026-48526, **JWT auth bypass via forged token**), `cryptography>=48.0.1` (GHSA-537c-gmf6-5ccf, vulnerable OpenSSL bundled in wheels), `starlette>=1.3.1` (CVE-2026-48818 SSRF/NTLM via UNC path, CVE-2026-54283 form-limit bypass — note this is a major starlette version, resolved cleanly under the pinned FastAPI), `python-multipart>=0.0.31` (CVE-2026-42561/53538/53539/53540, DoS), `pydantic-settings>=2.14.2` (GHSA-4xgf-cpjx-pc3j), `idna>=3.15`, `urllib3>=2.7.0`, `authlib>=1.6.12` (open-redirect fix on top of GHSA-jj8c-mmj3-mmgv), and `netmiko>=4.7.0`. Transitive floors are carried as explicit direct pins so a lock regen cannot silently regress them: `joserfc>=1.6.8` (CVE-2026-49852, HMAC verify accepts an empty key), `click>=8.3.3` (PYSEC-2026-2132), `msgpack>=1.2.1` (GHSA-6v7p-g79w-8964), `idna`, `urllib3`. The only `pip-audit` findings left are explicitly ignored with justification in the Security workflow — pip (a build tool, never invoked at runtime) and paramiko PYSEC-2026-2858 (no upstream fix; accepted residual risk, as SSH targets are operator-owned routers — see `SECURITY_MODEL.md`).
- **Query result cache is now scoped to the caller's role.** The Redis cache key previously keyed only on the query (device, type, target), so a result computed for a privileged caller — unfiltered, including more-specific prefixes — could be served from cache to a subsequent PUBLIC/guest hit on the same target within the TTL, bypassing the output filter. `_cache_key` now incorporates the caller role (`anonymous` for unauthenticated callers), giving each role an isolated cache namespace. Closes a cross-role leak present since v1.3.x that bare-IP lookups (below) made easier to trigger.
...
- **`BGPEEK_ACCEPT_BARE_IP`** (default `true`) — accept bare host addresses (`8.8.8.8`, `2001:4860:4860::8888`) as BGP-route targets. The router performs a longest-prefix-match and returns whichever covering prefix exists; the output filter still enforces `BGPEEK_MAX_PREFIX_V4/V6` on the response, so the prefix-length invariant holds for unprivileged roles (a more-specific match is stripped and `lpm_hidden: true` is set so the UI can render a hint). Dispatch is vendor-aware — 6WIND `show bgp ipv4 ip`, Junos `show route … best detail` (dropping `exact`) — for platforms whose standard prefix command is exact-match only. Set to `false` to reject bare addresses at input. **Privacy note:** where `MAX_PREFIX` is used as a boundary to hide customer /25–/32 allocations, bare-IP queries can act as an existence oracle; set `false` to close it. See *Bare-IP lookup and the existence-oracle consideration* in the configuration docs.
- **`BGPEEK_TRACEROUTE_NO_RESOLVE`** (default `false`) — append the platform's no-reverse-DNS flag to traceroute (e.g. Junos `traceroute … no-resolve`). Implemented via a per-`(platform, query_type)` optional-flags overlay: a platform that does not declare support for a flag degrades to a no-op rather than an error, so this deploy-wide knob is safe across a heterogeneous device fleet. Per-platform source-IP flags moved onto the same overlay.
- **Delegated user-management endpoints for NOC panels.** `PATCH /api/users/{id}` (scope `users:update`) edits role/email/enabled; `POST /api/users/{id}/password` (scope `users:reset_password`) resets a password and stamps `users.sessions_valid_after` (migration `0011`) so every existing JWT for that user is invalidated immediately. A last-admin-lockout guard refuses to disable, demote, or delete the final enabled admin. A new `reset_password` audit action records resets. `POST /api/users/local` now authorises by role-subsumption (a `users:create` caller may mint users at or below its own role) instead of scope-subsumption, which previously blocked every scoped caller.
...
- **Username validation tightened.** Local usernames now require a minimum length of 3 and must match `^[A-Za-z0-9][A-Za-z0-9._+@-]*[A-Za-z0-9]$` on creation and update. `LoginRequest` is deliberately exempt (it must still accept pre-existing rows) and the LDAP/OIDC upsert paths are unaffected (the identity provider is authoritative for naming). The admin create-user form gains matching `minlength` / `pattern` hints.
- **Shared-result access flow no longer renders a blank 200.** `/result/{id}` previously returned a 200 empty page for both anonymous callers and logged-in callers without rights. Anonymous callers are now redirected to `/auth/login?next=/result/{id}` (303); a logged-in caller lacking rights — or a missing id — gets a `404`, so status codes do not leak result existence. The web-login handler now honours a safe `?next=` redirect target.
...
- **Region filter no longer throws on change.** The `<optgroup>` change handler called `Array.from(optgroup.options)`, but `HTMLOptGroupElement` has no `.options` property — every region change raised a `TypeError` and broke the dependent device dropdown. It now iterates `optgroup.querySelectorAll('option')`. (Fallout from the v1.4.0 CSP inline-script extraction; the dropdown chevron was also restyled to match the query-type control.)
- **Webhook logging no longer swallows events.** Three webhook log calls passed `event=` as a structlog keyword, colliding with structlog's reserved `event` field (the log message) and raising `TypeError` inside the fire-and-forget dispatch path. Renamed to `webhook_event=`.
- **Query audit rows record the real client IP behind a reverse proxy.** Query endpoints logged the proxy's socket address instead of the client's. Audit now uses the shared `get_client_ip` helper, which trusts `X-Forwarded-For` only for requests arriving from `BGPEEK_TRUSTED_PROXIES` — an untrusted source still logs its socket peer, so the field cannot be spoofed.
...
[Unreleased]: https://github.com/xeonerix/bgpeek/compare/v1.4.1...HEAD
[1.4.1]: https://github.com/xeonerix/bgpeek/compare/v1.4.0...v1.4.1
